<?php
    ...
    class iconshopAdminController extends iconshop {
        ...
        function procIconshopAdminConfig() {
            ...
            $iconshop_config->member_info_print = ($args->member_info_print == "Y")? "Y" : "N";
            $iconshop_config->icon_width = (int)$args->icon_width;
            $iconshop_config->icon_height = (int)$args->icon_height;
            $iconshop_config->send_fee = (int)$args->send_fee;
            $iconshop_config->sell_per = (int)$args->sell_per;
            $iconshop_config->new_hour = (int)$args->new_hour;
            $iconshop_config->member_max_count = (int)$args->member_max_count;
            $iconshop_config->log_save_day = (int)$args->log_save_day;
            $iconshop_config->sender_srl = (int)$args->sender_srl;
            $iconshop_config->item_delete_event = ($args->item_delete_event == "Y")? "Y" : "N";
            $iconshop_config->item_delete_title = $args->item_delete_title;
            $iconshop_config->item_delete_message = $args->item_delete_message;

            // 수수료
            if($iconshop_config->send_fee > 100) $iconshop_config->send_fee = 0;

            // 되팔기시 돌려줄금액
            if($iconshop_config->sell_per > 100) $iconshop_config->sell_per = 100;

            // 저장
            $oModuleController = &getController('module');
            $oModuleController->insertModuleConfig('iconshop', $iconshop_config);
